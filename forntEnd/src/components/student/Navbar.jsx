    ...
    <div className={`flex items-center justify-between px-4 sm:px-10 md:px-14 lg:px-36 border-b border-gray-500 py-4 ${isCoursePage ? 'bg-white': 'bg-cyan-100/70'}`}>
      ...
      <div className='hidden md:flex items-center gap-5 text-grey-500'>
        <div className='flex items-center gap-5'>
          { user && <><button onClick={()=>{Navigate('/educator')}}>{isEducator?'Educator Dashboard':'Become Educator'}</button> | <Link to={'/my-enrollement'}>My Enrollement</Link></>}
        </div>
          { user? <UserButton/>:
      ...
      <div className='md:hidden flex items-center gap-2 sm:gap-5 text-gray-500'>
        <div className='flex items-center gap-1 sm:gap-2 max-sm:text-xs '>{ user && <><button onClick={()=>{Navigate('/educator')}}>{isEducator?'Educator Dashboard':'Become Educator'}</button> | <Link to={'/my-enrollement'}>My Enrollement</Link></>}</div>        
        {user? <UserButton/>:
