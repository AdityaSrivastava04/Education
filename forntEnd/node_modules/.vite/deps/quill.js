...
// node_modules/quill/core/quill.js
var debug = logger_default("quill");
...
var _Quill = class _Quill {
  ...
  constructor(container) {
    ...
    this.emitter.on(emitter_default.events.SCROLL_EMBED_UPDATE, (blot, delta) => {
      ...
      modify.call(this, () => {
        const change = new import_quill_delta.default().retain(blot.offset(this)).retain({
  ...
  format(name, value) {
    ...
    return modify.call(this, () => {
      ...
      let change = new import_quill_delta.default();
  ...
  setContents(delta) {
    ...
    return modify.call(this, () => {
      delta = new import_quill_delta.default(delta);
  ...
  setText(text) {
    let source = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : emitter_default.sources.API;
    const delta = new import_quill_delta.default().insert(text);
...
__publicField(_Quill, "imports", {
  delta: import_quill_delta.default,
...
function expandConfig(containerOrSelector, options) {
  ...
  if (registry) {
    ...
    registry = options.formats ? createRegistryWithFormats_default(options.formats, config4.registry, debug) : config4.registry;
  }
  return {
    ...config4,
    registry,
...
var Scroll = class extends ScrollBlot$1 {
  ...
  insertContents(index, delta) {
    const renderBlocks = this.deltaToRenderBlocks(delta.concat(new import_quill_delta2.default().insert("\n")));
    ...
    if (first) {
      ...
      const delta2 = first.type === "block" ? first.delta : new import_quill_delta2.default().insert({
      ...
      const attributes = import_quill_delta2.AttributeMap.diff(formats, first.attributes) || {};
  ...
  deltaToRenderBlocks(delta) {
    const renderBlocks = [];
    let currentBlockDelta = new import_quill_delta2.default();
    delta.forEach((op) => {
      ...
      if (typeof insert === "string") {
        ...
        splitted.slice(0, -1).forEach((text) => {
          ...
          currentBlockDelta = new import_quill_delta2.default();
        });
...
var debug2 = logger_default("quill:keyboard");
...
var Keyboard = class _Keyboard extends module_default {
  ...
  addBinding(keyBinding) {
    ...
    if (binding == null) {
      debug2.warn("Attempted to add invalid keyboard binding", binding);
      return;
    }
  ...
  handleBackspace(range, context) {
    ...
    let delta = new import_quill_delta3.default().retain(range.index - length).delete(length);
    if (context.offset === 0) {
      ...
      if (prev) {
        ...
        if (!isPrevLineEmpty) {
          ...
          formats = import_quill_delta3.AttributeMap.diff(curFormats, prevFormats) || {};
          if (Object.keys(formats).length > 0) {
            const formatDelta = new import_quill_delta3.default().retain(range.index + line.length() - 2).retain(1, formats);
  ...
  handleDelete(range, context) {
    ...
    let formats = {};
    const [line] = this.quill.getLine(range.index);
    let delta = new import_quill_delta3.default().retain(range.index).delete(length);
    if (context.offset >= line.length() - 1) {
      const [next] = this.quill.getLine(range.index + 1);
      if (next) {
        const curFormats = line.formats();
        const nextFormats = this.quill.getFormat(range.index, 1);
        formats = import_quill_delta3.AttributeMap.diff(curFormats, nextFormats) || {};
        if (Object.keys(formats).length > 0) {
  ...
  handleEnter(range, context) {
    ...
    const delta = new import_quill_delta3.default().retain(range.index).delete(range.length).insert("\n", lineFormats);
    this.quill.updateContents(delta, Quill.sources.USER);
  ...
  bindings: {
    ...
    "checklist enter": {
      ...
      handler(range) {
        ...
        const delta = new import_quill_delta3.default().retain(range.index).insert("\n", formats).retain(line.length() - offset - 1).retain(1, {
          list: "unchecked"
        });
        this.quill.updateContents(delta, Quill.sources.USER);
        this.quill.setSelection(range.index + 1, Quill.sources.SILENT);
    ...
    "table enter": {
      ...
      handler(range) {
        ...
        if (module) {
          ...
          if (shift < 0) {
            const delta = new import_quill_delta3.default().retain(index).insert("\n");
            this.quill.updateContents(delta, Quill.sources.USER);
    ...
    "list autofill": {
      ...
      handler(range, context) {
        ...
        this.quill.history.cutoff();
        const delta = new import_quill_delta3.default().retain(range.index - offset).delete(length + 1).retain(line.length() - 2 - offset).retain(1, {
          list: value
        });
        this.quill.updateContents(delta, Quill.sources.USER);
        this.quill.history.cutoff();
    ...
    "code exit": {
      ...
      handler(range) {
        ...
        while (cur != null && cur.length() <= 1 && cur.formats()["code-block"]) {
          ...
          numLines -= 1;
          if (numLines <= 0) {
            const delta = new import_quill_delta3.default().retain(range.index + line.length() - offset - 2).retain(1, {
              "code-block": null
            }).delete(1);
            this.quill.updateContents(delta, Quill.sources.USER);
            this.quill.setSelection(range.index - 1, Quill.sources.SILENT);
...
function deleteRange(_ref2) {
  ...
  if (lines.length > 1) {
    ...
    formats = import_quill_delta3.AttributeMap.diff(lastFormats, firstFormats) || {};
  }
...
// node_modules/quill/modules/clipboard.js
var debug3 = logger_default("quill:clipboard");
...
var Clipboard = class extends module_default {
  ...
  convert(_ref2) {
    ...
    if (formats[CodeBlock.blotName]) {
      return new import_quill_delta4.default().insert(text || "", {
        [CodeBlock.blotName]: formats[CodeBlock.blotName]
      });
    }
    if (!html) {
      return new import_quill_delta4.default().insert(text || "", formats);
    }
    const delta = this.convertHTML(html);
    if (deltaEndsWith(delta, "\n") && (delta.ops[delta.ops.length - 1].attributes == null || formats.table)) {
      return delta.compose(new import_quill_delta4.default().retain(delta.length() - 1).delete(1));
    }
    return delta;
  }
  normalizeHTML(doc) {
  ...
  dangerouslyPasteHTML(index, html) {
    ...
    if (typeof index === "string") {
      ...
      this.quill.updateContents(new import_quill_delta4.default().retain(index).concat(paste), source);
  ...
  onPaste(range, _ref3) {
    ...
    debug3.log("onPaste", pastedDelta, {
      text,
      html
    });
    const delta = new import_quill_delta4.default().retain(range.index).delete(range.length).concat(pastedDelta);
    this.quill.updateContents(delta, Quill.sources.USER);
...
function applyFormat(delta, format, value, scroll) {
  ...
  }, new import_quill_delta4.default());
}
...
function matchBlot(node, delta, scroll) {
  ...
  if (match.prototype instanceof EmbedBlot$1) {
    ...
    if (value != null) {
      ...
      return new import_quill_delta4.default().insert(embed, match.formats(node, scroll));
    }
  } else {
...
function matchStyles(node, delta, scroll) {
  ...
  if (parseFloat(style.textIndent || 0) > 0) {
    return new import_quill_delta4.default().insert("	").concat(delta);
  }
  return delta;
}
...
Uploader.DEFAULTS = {
  ...
  handler(range, files) {
    ...
    Promise.all(promises).then((images) => {
      ...
      }, new import_quill_delta5.default().retain(range.index).delete(range.length));
...
var Input = class extends module_default {
  ...
  replaceText(range) {
    ...
    if (text) {
      ...
      this.quill.updateContents(new import_quill_delta6.default().retain(range.index).insert(text, formats), Quill.sources.USER);
    } else {
      this.deleteRange(range);
    }
...
`;
    ...
    if (forced || this.forceNext || this.cachedText !== text) {
      if (text.trim().length > 0 || this.cachedText == null) {
        ...
        }, new import_quill_delta8.default());
...
var Syntax = class extends module_default {
  ...
  highlightBlot(text) {
    ...
    return traverse(this.quill.scroll, container, [(node, delta) => {
      ...
      if (value) {
        return delta.compose(new import_quill_delta8.default().retain(delta.length(), {
...
var Table = class extends module_default {
  ...
  insertTable(rows, columns) {
    ...
    }, new import_quill_delta9.default().retain(range.index));
    this.quill.updateContents(delta, Quill.sources.USER);
...
var debug4 = logger_default("quill:toolbar");
var Toolbar = class extends module_default {
  constructor(quill, options) {
    ...
    if (!(this.container instanceof HTMLElement)) {
      debug4.error("Container required for toolbar", this.options);
      return;
    }
  ...
  attach(input) {
    ...
    if (this.handlers[format] == null && this.quill.scroll.query(format) == null) {
      debug4.warn("ignoring attaching to nonexistent format", format, input);
      return;
    }
    ...
    input.addEventListener(eventName, (e) => {
      ...
      if (this.handlers[format] != null) {
        ...
        if (!value) return;
        this.quill.updateContents(new import_quill_delta10.default().retain(range.index).delete(range.length).insert({
          [format]: value
        }), Quill.sources.USER);
      } else {
...
var export_AttributeMap = import_quill_delta7.AttributeMap;
var export_Delta = import_quill_delta7.default;
var export_Op = import_quill_delta7.Op;
var export_OpIterator = import_quill_delta7.OpIterator;
export {
